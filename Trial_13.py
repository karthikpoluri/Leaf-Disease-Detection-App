    ...
    def predicting_uploaded_image(self):
        ...
        if predictions[0,0] == 1:
            self.dialog = MDDialog(title='Your Prediction',
                                   text="Ditected dicease is {}".format("Tomato___Bacterial_spot"), size_hint=(0.9, 1),
                                   buttons=[back_button])
            self.dialog.open()
        elif predictions[0,0] == 0:
            self.dialog = MDDialog(title='Your Prediction', text="Please eneter the right image",
...
class CameraLayout(MDBoxLayout):
    ...
    def predicting_captured_image(self):
        ...
        if predictions[:0] == 1.0:
            ...
            self.dialog = MDDialog(title='Your Prediction', text="Ditected dicease is {}".format("unrelated picture"),
                                   size_hint=(0.9, 1), buttons=[back_button])
            self.dialog.open()
